...
proc init_known*(count: csize; cells: cstringArray)
...
proc sscanf(c: cstring, frmt: cstring): cint {.varargs, importc, header: "<stdio.h>".}
...
proc init_known*(count: csize; cells: cstringArray) =
  var c: cint = 0
  while c < count:
    var cell: cstring = cells[c]
    var
      i: cint
      j: cint
      n: cint
    let format: cstring = "%1d%1d%1d"
    if sscanf(cell, format, addr i, addr j, addr n) > 0:
      set_cell(i - 1, j - 1, n)
      known[i - 1][j - 1] = 1
      quit("bad input token:" & $cell)
    inc(c)
